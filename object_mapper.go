...
// MapWithConverters - map values from source to target, and use builder functions passed
func MapWithConverters(source interface{}, target interface{}, builders map[string]TypeConverterFn) error {
	if err := verifyParameters(source, target); err != nil {
		return err
	}

	// shallow copy TODO remove
	var builderMap = defaultTypeConvertMap
...
func mapValues(sourceValue reflect.Value, targetValue reflect.Value, builders map[string]TypeConverterFn) (interface{}, error) {
	...
	case reflect.Ptr:
		...
		if fn, ok := defaultTypeConvertMap[targetValue.Type().Elem().String()]; ok {
	...
	case reflect.Struct:
		...
		for i := 0; i < numFields; i++ {
			...
			if fn, ok := defaultTypeConvertMap[targetFieldValue.Type().String()]; ok {
